...
- Existing violations are not a reason to omit a rule or add a permanent allowlist. If the rule represents the intended architecture but current code violates it, configure that architecture entry as `candidate` so Cleanup Scan creates an issue without breaking blocking checks.
